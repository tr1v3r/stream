...
//
//	G1 near-free Filter+Map, Parallel(4) unordered <= 4x serial
//	   (v2 measures ~2.4x; the v1 per-op-pool regression sat at 14-21x)
//	G2 near-free, Parallel(4).Ordered()            <= 6x serial
//	   (v2 measures ~2.9x)
//	G3 heavy work, Parallel(4) >= 1.5x faster than serial
//	   (v2 measures ~3.6x; a fusion failure collapses to ~1x)
//
...
func TestBenchmarkGate(t *testing.T) {
	...
	if r := float64(unordered) / float64(serial); r > 4.0 {
		t.Errorf("G1 FAIL: near-free Parallel(4) overhead %.2fx serial (gate <= 4.0x; a v1-style per-op-pool regression is 14-21x)", r)
	} else {
		t.Logf("G1 pass: unordered %.2fx serial", float64(unordered)/float64(serial))
	}
	if r := float64(ordered) / float64(serial); r > 6.0 {
		t.Errorf("G2 FAIL: ordered overhead %.2fx serial (gate <= 6.0x)", r)
	} else {
